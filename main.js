...
products.addEventListener('click', function(e) {
  ...
  if (e.target.classList.contains('add-to-cart')) {
    let alreadyInCart = cart.some(
      item => item.name === e.target.closest('.item').getAttribute('data-name')
    );
    if (alreadyInCart) {
      cart = cart.map(item => {
      ...
      cart.push({
        name: e.target.closest('.item').getAttribute('data-name'),
        price: e.target.closest('.item').getAttribute('data-price'),
        qty: 1
      });
    }
    renderCart();
    ...
    cart = cart.filter(
      item => item.name !== e.target.parentElement.getAttribute('data-name')
    );
    renderCart();
  }
});
...
const renderCart = function() {
  ...
  for (let i = 0; i < cart.length; i++) {
    html +=
      ...
      '  ' +
      '<input type="image" src="redx.png" class="delete">' +
      '</div>';
  }
...
const totalCart = function() {
  ...
  document.getElementsByClassName('total')[0].innerHTML = total.reduce(
    (a, b) => a + b,
    0
  );
};
